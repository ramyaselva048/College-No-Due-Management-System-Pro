...
apiRouter.post('/auth/login', (req: Request, res: Response) => {
  ...
  if (user.role === 'STUDENT') {
    ...
    userInfo.full_name = user.full_name || 'Dr. T. Senthilvel (Principal / Admin)';
    userInfo.username = user.username || user.email.split('@')[0];
  }
...
apiRouter.get('/student/summary', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  ...
  let cert = db.certificates.slice().reverse().find(c => c.student_id === student.id && c.is_valid);
  const dept = db.departments.find(d => d.id === student.department_id);
  const course = db.courses.find(c => c.id === student.course_id);
  ...
  if (activeReq) {
    ...
    const clearedNodesCount = (reqSubs.length - pendingSubs.length) + (reqLabs.length - pendingLabsList.length) + (reqCommon.length - pendingCommonList.length);
    ...
    activeReqOut = {
      ...
      exam_type: activeReq.exam_type || 'CIAT - I',
      academic_year: activeReq.academic_year || '2025-26',
      year: activeReq.year || student.year || 4,
      semester: targetSem,
      student_type: activeReq.student_type || student.student_type || 'day_scholar',
      attendance_percentage: activeReq.attendance_percentage ?? student.attendance_percentage ?? 98,
      subjects: reqSubs,
      labs: reqLabs,
      common_nodes: reqCommon,
      signatories: activeReq.signatories || defaultSignatoriesForStudent(activeReq),
      all_clearance_nodes_cleared: allClearanceNodesCleared,
      total_nodes_count: totalNodesCount,
      cleared_nodes_count: clearedNodesCount,
      pending_nodes_count: totalNodesCount - clearedNodesCount,
      clearance_percentage: totalNodesCount > 0 ? Math.round((clearedNodesCount / totalNodesCount) * 100) : 100
    };
  }
...
// ----------------------------------------------------
function findStaffForFacultyName(facultyName?: string) {
  if (!facultyName) return null;
  const nameLower = facultyName.toLowerCase();
  return db.staff.find(s =>
    s.full_name.toLowerCase() === nameLower ||
    nameLower.includes(s.full_name.toLowerCase()) ||
    s.full_name.toLowerCase().includes(nameLower)
  ) || null;
}

function defaultSubjectsForStudent(student?: any, semester?: number, initialPending: boolean = true): Array<{ slot: string; name: string; dues_status: string; faculty_name?: string; signature_date?: string; code?: string; faculty_id?: number; faculty_email?: string }> {
  ...
  if (dbSubjects.length > 0) {
    return dbSubjects.slice(0, 6).map((c, idx) => {
      const staff = findStaffForFacultyName(c.faculty_name);
      return {
        ...
        faculty_id: staff ? staff.id : undefined,
        faculty_email: staff ? staff.email : undefined,
...
function defaultLabsForStudent(student?: any, semester?: number, initialPending: boolean = true): Array<{ slot: string; name: string; dues_status: string; faculty_name?: string; signature_date?: string; code?: string; faculty_id?: number; faculty_email?: string }> {
  const deptId = student?.department_id || 1;
  const targetSem = Number(semester) || student?.semester || (student?.year ? student.year * 2 - 1 : 7);
  const targetYear = student?.year || Math.ceil(targetSem / 2);
  const dateStr = initialPending ? '-' : new Date().toLocaleDateString('en-GB');
...
export function checkAndIssueCertificateIfCleared(reqId: number): CertificateRecord | null {
  ...
  // Automated certificate issuance is completely disabled.
  // Certificates can ONLY be reviewed and issued manually by the Principal (Admin)
  // after staff clearances and HOD digital sign-off.
  return db.certificates.find(c => c.request_id === reqId && c.is_valid) || null;
}

export function getEnrichedCertificateData(cert: CertificateRecord) {
  ...
  const targetSem = reqRecord?.semester || st?.semester || (st?.year === 4 ? 7 : (st?.year ? st.year * 2 - 1 : 7));
  const targetYear = reqRecord?.year || st?.year || Math.ceil(targetSem / 2);
  const issueDateStr = cert.issued_at
    ? new Date(cert.issued_at).toLocaleDateString('en-GB')
    : new Date().toLocaleDateString('en-GB');

  // Populate subjects (theory courses)
  const rawSubjects = (reqRecord?.subjects && reqRecord.subjects.length > 0)
    ? reqRecord.subjects
    : defaultSubjectsForStudent(st, targetSem, false);

  // Populate labs (practical courses)
  const rawLabs = (reqRecord?.labs && reqRecord.labs.length > 0)
    ? reqRecord.labs
  ...
  const subjects = rawSubjects.map((s, idx) => ({
    ...
    name: s.name,
    code: s.code || `CS3${targetSem}0${idx + 1}`,
    dues_status: cert.is_valid ? 'No Dues' : (s.dues_status || 'Pending'),
    faculty_name: s.faculty_name || 'Faculty In-Charge',
    faculty_email: (s as any).faculty_email,
    signature_date: (!s.signature_date || s.signature_date === '-') ? issueDateStr : s.signature_date
  }));

  const labs = rawLabs.filter(l => l.name !== '-').map((l, idx) => ({
    slot: l.slot || `Lab ${idx + 1}`,
    name: l.name,
    code: l.code || `CS3${targetSem}1${idx + 1}`,
    dues_status: cert.is_valid ? 'No Dues' : (l.dues_status || 'Pending'),
    faculty_name: l.faculty_name || 'Lab In-Charge',
    faculty_email: (l as any).faculty_email,
    signature_date: (!l.signature_date || l.signature_date === '-') ? issueDateStr : l.signature_date
  }));
  ...
  const common_nodes = rawCommon.map((c, idx) => {
    ...
    return {
      ...
      dues_status: cert.is_valid ? (isExempt ? c.dues_status : 'No Dues') : (c.dues_status || 'Pending'),
      faculty_name: c.faculty_name || 'Officer In-Charge',
      faculty_email: c.faculty_email,
      signature_date: (!c.signature_date || c.signature_date === '-') ? issueDateStr : c.signature_date,
      requirement_description: c.requirement_description || 'All institutional dues cleared'
    };
  });

  const academicYearStr = reqRecord?.academic_year || (st ? `${st.admission_year}-${st.admission_year + (course?.duration || 4)}` : '2025-26');
  const examTypeStr = reqRecord?.exam_type || 'CIAT - I / End Semester Examinations';
  ...
  return {
    ...
    department_code: dept ? dept.code : 'ENGG',
    ...
    labs,
    common_nodes,
    signatories: reqRecord?.signatories || defaultSignatoriesForStudent(reqRecord),
    request: reqRecord
  };
}

// Dedicated Student Clearance Nodes View Endpoint (Only academic nodes allocated by HOD + common institutional nodes)
apiRouter.get('/student/clearance-nodes', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  const student = req.studentProfile!;
  const activeReq = db.noDueRequests.slice().reverse().find(r => r.student_id === student.id && r.status !== 'rejected') || db.noDueRequests.slice().reverse().find(r => r.student_id === student.id);
  const targetSem = activeReq?.semester || student.semester || (student.year === 4 ? 7 : student.year * 2 - 1);
  const targetYear = activeReq?.year || student.year || 4;
  ...
  const pendingCount = pendingTheory.length + pendingLabs.length + pendingCommon.length;
  const clearedCount = totalNodesCount - pendingCount;
  const allCleared = pendingCount === 0;
  ...
  res.json({
    ...
    active_request: activeReq ? {
      ...
      submitted_at: activeReq.submitted_at,
      attendance_percentage: activeReq.attendance_percentage ?? student.attendance_percentage ?? 98,
      hod_endorsed: Boolean(activeReq.signatories?.hod?.signed),
      hod_name: activeReq.signatories?.hod?.name || null,
      principal_issued: Boolean(activeReq.signatories?.principal?.signed)
    } : null,
    academic_nodes: {
      theory: academicTheory,
      labs: academicLabs
    },
    common_nodes: commonNodes,
    stats: {
      total_nodes: totalNodesCount,
      cleared_nodes: clearedCount,
      pending_nodes: pendingCount,
      percentage: totalNodesCount > 0 ? Math.round((clearedCount / totalNodesCount) * 100) : 100,
...
// Student Check Certificate status endpoint (manual Principal issuance protocol)
apiRouter.post('/certificates/my/claim', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  const student = req.studentProfile!;
  const activeReq = db.noDueRequests.slice().reverse().find(r => r.student_id === student.id);
  if (!activeReq) {
    return res.status(400).json({ detail: 'No clearance application found. Please initiate a clearance request first.' });
  }

  const existingCert = db.certificates.find(c => c.request_id === activeReq.id && c.is_valid);
  if (existingCert) {
    return res.json({
      success: true,
      message: 'Certificate has already been officially issued by the Principal.',
  ...
  if (pending.length > 0) {
    return res.status(400).json({
      detail: `Cannot generate certificate. You still have ${pending.length} uncleared clearance node(s). Please clear these first with the respective allocated staff: ${pending.slice(0, 3).join(', ')}${pending.length > 3 ? '...' : ''}`,
      pending_nodes: pending
    });
  }

  if (!activeReq.signatories?.hod?.signed) {
    return res.status(400).json({
      detail: 'All academic subjects & common nodes are cleared! Your application is currently awaiting formal HOD acceptance and digital endorsement.',
      status: 'pending_hod'
    });
  }
...
apiRouter.post('/no-due-requests', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  ...
  const allocatedItems = [...newReq.subjects, ...newReq.labs];
  for (const item of allocatedItems) {
    if (item.faculty_name && item.name !== '-') {
      const staffMember = db.staff.find(s =>
        s.id === item.faculty_id ||
        s.full_name.toLowerCase() === item.faculty_name.toLowerCase() ||
        item.faculty_name.toLowerCase().includes(s.full_name.toLowerCase()) ||
        s.full_name.toLowerCase().includes(item.faculty_name.toLowerCase())
      );
      if (staffMember) {
        db.createNotification(
          staffMember.user_id,
          'Subject Clearance Application Submitted',
          `Student ${student.full_name} (${student.register_number}) applied for clearance in ${item.name} (${item.slot}). Please verify and accept.`,
          'info'
        );
      }
    }
  ...
  res.json({
    ...
    labs: newReq.labs,
    common_nodes: newReq.common_nodes,
...
apiRouter.patch('/no-due-requests/:id', authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  if (status === 'approved') {
    ...
    const pendingCommon = common.filter((c: any) => !isNodeClearedStatus(c.dues_status));

    const pendingList: string[] = [
      ...pendingSubjects.map((s: any) => `${s.slot}: ${s.name || s.title || s.subject_name}`),
      ...pendingLabs.map((l: any) => `${l.slot}: ${l.name}`),
      ...pendingCommon.map((c: any) => `${c.slot || 'Common'}: ${c.name}`)
    ];
    ...
    if (pendingList.length > 0) {
      if (isAdmin) {
        // Admin / Principal executive clearance: endorse and clear all remaining nodes under institutional authority
        const todayStr = new Date().toLocaleDateString('en-GB');
        subjects.forEach((s: any) => {
          if (!isNodeClearedStatus(s.dues_status)) {
            s.dues_status = 'Cleared';
            s.staff_signature = 'Approved (Principal Authority)';
            s.date = todayStr;
          }
        });
        labs.forEach((l: any) => {
          if (l.name !== '-' && !isNodeClearedStatus(l.dues_status)) {
            l.dues_status = 'Cleared';
            l.staff_signature = 'Approved (Principal Authority)';
            l.date = todayStr;
          }
        });
        common.forEach((c: any) => {
          if (!isNodeClearedStatus(c.dues_status)) {
            c.dues_status = 'Cleared';
            c.staff_signature = 'Approved (Principal Authority)';
            c.date = todayStr;
          }
        });
        r.subjects = subjects;
        r.labs = labs;
        r.common_nodes = common;
      } else {
        return res.status(400).json({
          detail: `Cannot approve clearance request. All clearance nodes must be cleared by allocated staff first. ${pendingList.length} pending node(s): ${pendingList.join(', ')}.`
        });
      }
    }
    ...
    if (!hasHODEndorsed) {
      if (isAdmin) {
        const todayStr = new Date().toLocaleDateString('en-GB');
        r.signatories = r.signatories || defaultSignatoriesForStudent(r);
        r.signatories.hod = {
          signed: true,
          name: r.signatories.hod?.name || 'Head of Department',
          date: todayStr,
          status: 'endorsed',
          remarks: 'Endorsed under Institutional Administrative Authority'
        };
        r.hod_approved_at = new Date().toISOString();
        r.hod_approved_by = req.user!.id;
        r.hod_name = r.signatories.hod?.name || 'Head of Department';
      } else {
        return res.status(400).json({
          detail: 'Cannot approve clearance request. Head of Department (HOD) endorsement must be completed before Principal approval.'
        });
      }
    }

    const studentDues = db.dueRecords.filter(d => d.student_id === r.student_id && d.status !== 'cleared' && d.status !== 'waived' && Number(d.amount) > 0);
    if (studentDues.length > 0) {
      const totalDue = studentDues.reduce((sum, d) => sum + Number(d.amount), 0);
      return res.status(400).json({
        detail: `Cannot approve clearance request. Student has ₹${totalDue} in unpaid financial dues. All student dues must be cleared first.`
      });
    }

    const todayStr = new Date().toLocaleDateString('en-GB');
    r.signatories = r.signatories || defaultSignatoriesForStudent(r);
    r.signatories.principal = {
      signed: true,
      name: 'Dr. T. Senthilvel (Principal / Admin)',
      date: todayStr,
      status: 'approved',
      remarks: remarks || 'All departmental clearances and HOD endorsement verified. Approved by Principal.'
    };
...
apiRouter.post(['/certificates/request/:request_id/issue', '/certificates/issue/:request_id'], authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  // 1. Strict Admin Approval requirement:
  ...
  if (studentDues.length > 0) {
    const totalDue = studentDues.reduce((sum, d) => sum + Number(d.amount), 0);
    return res.status(400).json({
      detail: `Cannot issue certificate. Student has ₹${totalDue} in unpaid financial dues. All student dues must be cleared first.`
    });
  }

  // 3. Ensure nodes are marked certified under approved clearance
  const todayStr = new Date().toLocaleDateString('en-GB');
  const subjects = r.subjects && r.subjects.length > 0 ? r.subjects : defaultSubjectsForStudent(st, r.semester, true);
  const labs = r.labs && r.labs.length > 0 ? r.labs : defaultLabsForStudent(st, r.semester, true);
  ...
  subjects.forEach((s: any) => {
    if (!isNodeClearedStatus(s.dues_status)) {
      s.dues_status = 'Cleared';
      s.staff_signature = s.staff_signature || 'Approved (Principal Authority)';
  ...
  labs.forEach((l: any) => {
    if (l.name !== '-' && !isNodeClearedStatus(l.dues_status)) {
      l.dues_status = 'Cleared';
      l.staff_signature = l.staff_signature || 'Approved (Principal Authority)';
  ...
  common.forEach((c: any) => {
    if (!isNodeClearedStatus(c.dues_status)) {
      c.dues_status = 'Cleared';
      c.staff_signature = c.staff_signature || 'Approved (Principal Authority)';
  ...
  if (!r.signatories.principal?.signed) {
    r.signatories.principal = {
      signed: true,
      name: 'Dr. T. Senthilvel (Principal / Admin)',
      date: todayStr,
...
apiRouter.get('/certificates/my', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  ...
  const result = certs.map(c => getEnrichedCertificateData(c));
  res.json(result);
});

// Student Reset Certificate Endpoint
apiRouter.post('/certificates/my/reset', authMiddleware, requireRole(['STUDENT']), (req: AuthRequest, res: Response) => {
  const student = req.studentProfile!;
  const cert = db.certificates.slice().reverse().find(c => c.student_id === student.id && c.is_valid);
  if (!cert) {
    return res.status(404).json({ detail: 'No issued certificate found to reset.' });
  }

  const reqObj = db.noDueRequests.find(r => r.id === cert.request_id);
  if (reqObj) {
    reqObj.subjects = defaultSubjectsForStudent(student, reqObj.semester || 7, false);
    reqObj.labs = defaultLabsForStudent(student, reqObj.semester || 7, false);
    reqObj.common_nodes = defaultCommonNodesForStudent(student, false);
    db.saveToFile();
  }

  const enriched = getEnrichedCertificateData(cert);
  db.logAudit(req.user!.id, req.user!.email, 'CERTIFICATE_RESET', 'CERTIFICATE', cert.id, null, { certificate_number: cert.certificate_number }, getClientIp(req));
  res.json({ message: 'Certificate clearance view reset successfully', certificate: enriched });
});

// General Certificate Reset Endpoint
apiRouter.post(['/certificates/:id/reset', '/certificates/reset/:id'], authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  if (req.user!.role === 'STUDENT' && req.studentProfile && cert.student_id !== req.studentProfile.id) {
    return res.status(403).json({ detail: 'You are not authorized to reset this certificate' });
  }

  const st = db.students.find(s => s.id === cert.student_id);
  const reqObj = db.noDueRequests.find(r => r.id === cert.request_id);
  if (reqObj && st) {
    reqObj.subjects = defaultSubjectsForStudent(st, reqObj.semester || 7, false);
    reqObj.labs = defaultLabsForStudent(st, reqObj.semester || 7, false);
    reqObj.common_nodes = defaultCommonNodesForStudent(st, false);
    db.saveToFile();
  }

  const enriched = getEnrichedCertificateData(cert);
  db.logAudit(req.user!.id, req.user!.email, 'CERTIFICATE_RESET', 'CERTIFICATE', cert.id, null, { certificate_number: cert.certificate_number }, getClientIp(req));
  res.json({ message: 'Certificate data reset successfully', certificate: enriched });
});
...
apiRouter.get(['/certificates', '/certificates/all'], authMiddleware, requireRole(['STAFF', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const result = paginated.map(c => getEnrichedCertificateData(c));
...
apiRouter.get('/certificates/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  res.json(getEnrichedCertificateData(cert));
});
...
apiRouter.get('/certificates/:id/download', authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  const enriched = getEnrichedCertificateData(cert);
  const pdfBuffer = generateCertificatePdf(enriched);
...
// ----------------------------------------------------
apiRouter.get('/admin/profile', authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  const user = req.user!;
  res.json({
    id: user.id,
    full_name: user.full_name || 'Dr. T. Senthilvel (Principal / Admin)',
    username: user.username || user.email.split('@')[0],
    email: user.email,
    role: user.role,
    designation: 'Principal & Head of Institution',
    institution_name: 'College of Engineering (Autonomous)',
    office: 'Office of the Principal / Administration',
    is_active: user.is_active,
    created_at: user.created_at,
...
apiRouter.patch('/admin/profile', authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  const user = req.user!;
  ...
  if (username !== undefined) {
    ...
    if (cleanUsername.length > 0) {
      ...
      if (!/^[a-zA-Z0-9._-]+$/.test(cleanUsername)) {
        return res.status(400).json({ detail: 'Username can only contain letters, numbers, dots, hyphens, and underscores.' });
      }
      const existingUser = db.users.find(u => u.id !== user.id && u.username && u.username.toLowerCase() === cleanUsername);
      if (existingUser) {
        return res.status(400).json({ detail: 'This username is already taken. Please choose another username.' });
      }
      user.username = cleanUsername;
    }
  }
  ...
  if (full_name !== undefined) {
    if (!full_name.trim()) {
      return res.status(400).json({ detail: 'Administrator full name cannot be empty.' });
    }
...
apiRouter.post('/admin/change-password', authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  const user = req.user!;
  ...
  if (confirm_password && new_password !== confirm_password) {
    return res.status(400).json({ detail: 'New password and confirmation password do not match.' });
  }

  if (current_password && !verifyPassword(current_password, user.password_hash)) {
    return res.status(400).json({ detail: 'Current password entered is incorrect.' });
  }
...
apiRouter.get('/admin/students', authMiddleware, requireRole(['ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  if (req.query.course_id && req.query.course_id !== 'all') {
  ...
  if (req.query.semester && req.query.semester !== 'all') {
    students = students.filter(s => {
      const sem = s.semester || (s.year ? s.year * 2 - 1 : 1);
      return sem === Number(req.query.semester);
    });
  }
  if (req.query.section && req.query.section !== 'all') {
    students = students.filter(s => (s.section || 'A').toUpperCase() === String(req.query.section).toUpperCase());
  }
  const search = (req.query.search as string || '').toLowerCase().trim();
  if (search) {
    students = students.filter(s =>
  ...
  const paginated = hasLimit
    ? students.slice(skip, skip + Number(req.query.limit))
    : (skip > 0 ? students.slice(skip) : students);
...
apiRouter.get('/hod/clearance-nodes', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const courseType = req.query.course_type as string | undefined;

  let nodes = db.subjectCourses.filter(c => {
    if (courseType === 'common' || c.course_type === 'common') {
      return c.course_type === 'common';
    }
    return c.department_id === deptId;
  });
  
  if (req.query.year && courseType !== 'common') {
    nodes = nodes.filter(c => c.year === Number(req.query.year));
  }
  if (req.query.semester && courseType !== 'common') {
    nodes = nodes.filter(c => c.semester === Number(req.query.semester));
  }
  if (courseType) {
    nodes = nodes.filter(c => c.course_type === courseType);
  }

  const sorted = nodes.slice().sort((a, b) => {
    if (a.course_type === 'common' && b.course_type === 'common') {
...
apiRouter.post('/hod/clearance-nodes', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const { title, code, year, semester, course_type, slot, faculty_name, faculty_id, faculty_email, is_elective, applies_to, requirement_description, category_key } = req.body;

  const isCommon = course_type === 'common';
  if (!title || !code) {
    return res.status(400).json({ detail: 'Title and Code are required to allocate a clearance node.' });
  }
  if (!isCommon && (!year || !semester)) {
    return res.status(400).json({ detail: 'Title, Code, Year, and Semester are required to allocate an academic clearance node.' });
  }
  ...
  const newNode: SubjectCourseRecord = {
    ...
    department_id: isCommon ? 0 : deptId,
    year: isCommon ? 0 : Number(year),
    semester: isCommon ? 0 : Number(semester),
    course_type: course_type || 'theory',
    slot: slot || (course_type === 'common' ? `COM-${code.substring(0, 3).toUpperCase()}` : (course_type === 'lab' ? `Lab ${nextId % 4 + 1}` : `Sub ${nextId % 6 + 1}`)),
    faculty_name: faculty_name || (isCommon ? 'Officer In-Charge' : 'Department Faculty'),
    faculty_id: faculty_id ? Number(faculty_id) : undefined,
    faculty_email: faculty_email || undefined,
    is_elective: Boolean(is_elective),
    is_active: true,
    applies_to: applies_to || 'all',
    requirement_description: requirement_description?.trim() || undefined,
...
apiRouter.get('/hod/common-clearance-nodes', authMiddleware, requireRole(['HOD', 'ADMIN', 'STAFF']), (req: AuthRequest, res: Response) => {
  const commonNodes = ensureDefaultCommonNodes();

  const order = ['COM-LIB', 'COM-ACC', 'COM-TRN', 'COM-HST', 'COM-PED', 'COM-COE'];
  const sorted = commonNodes.slice().sort((a, b) => {
    const aIdx = order.indexOf(a.slot || '');
    const bIdx = order.indexOf(b.slot || '');
    if (aIdx !== -1 && bIdx !== -1) return aIdx - bIdx;
    if (aIdx !== -1) return -1;
    if (bIdx !== -1) return 1;
    return (a.slot || a.title).localeCompare(b.slot || b.title);
  });
...
apiRouter.post('/hod/common-clearance-nodes', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const newNode: SubjectCourseRecord = {
    ...
    is_active: true,
    applies_to: applies_to || 'all',
    requirement_description: requirement_description?.trim() || 'All departmental and institutional dues cleared',
    category_key: category_key || undefined,
    created_at: new Date().toISOString()
  };
...
apiRouter.put('/hod/common-clearance-nodes/:id', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  const id = Number(req.params.id);
  const node = db.subjectCourses.find(c => c.id === id);
  if (!node) {
...
apiRouter.post('/hod/common-clearance-nodes/populate-defaults', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  res.json({
    message: `Standard Common Institutional Clearance Nodes loaded successfully (${created.length} nodes for all students).`,
    nodes: created
  });
});
...
apiRouter.post('/hod/faculty', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  const hodStaff = req.staffProfile || db.staff.find(s => s.user_id === req.user!.id || s.email.toLowerCase() === req.user!.email.toLowerCase());
  const deptId = hodStaff?.department_id || 1;
  const { full_name, employee_id, email, password, phone, designation, is_active, subject_term } = req.body;
  ...
  const cleanTerm = subject_term ? String(subject_term).trim() : '';
...
apiRouter.patch('/hod/faculty/:id', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  if (subject_term !== undefined) staff.subject_term = String(subject_term).trim();
...
apiRouter.get('/hod/requests', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const enriched = requests.map(r => {
    ...
    return {
      ...
      all_common_cleared: pendingCommon.length === 0,
...
apiRouter.post('/hod/requests/:id/sign-off', authMiddleware, requireRole(['HOD', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  const subjects = noDueReq.subjects && noDueReq.subjects.length > 0 ? noDueReq.subjects : defaultSubjectsForStudent(student, noDueReq.semester, true);
  const labs = noDueReq.labs && noDueReq.labs.length > 0 ? noDueReq.labs : defaultLabsForStudent(student, noDueReq.semester, true);
  ...
  // User Requirement: Subject-wise HOD allocated staff must clear subjects first.
  // HOD cannot endorse if any allocated subject or lab is still pending.
  const pendingSubjects = subjects.filter((s: any) => !isNodeClearedStatus(s.dues_status));
  const pendingLabs = labs.filter((l: any) => l.name !== '-' && !isNodeClearedStatus(l.dues_status));

  if (pendingSubjects.length > 0 || pendingLabs.length > 0) {
    const pendingNames = [
      ...pendingSubjects.map((s: any) => `${s.slot}: ${s.name || s.subject_name || s.title}`),
      ...pendingLabs.map((l: any) => `${l.slot}: ${l.name}`)
    ];
    return res.status(400).json({
      detail: `Cannot endorse clearance. ${pendingNames.length} subject/lab node(s) are still pending verification from allocated faculty: ${pendingNames.join(', ')}. All allocated staff must clear their subjects before HOD can endorse.`
    });
  }

  noDueReq.subjects = subjects;
  noDueReq.labs = labs;
  noDueReq.common_nodes = common_nodes;
  ...
  noDueReq.signatories.hod = {
    ...
    date: todayStr,
    status: 'approved',
    remarks: req.body.remarks || 'All departmental subject clearances and lab records verified and endorsed by HOD.'
  };
  ...
  if (student) {
    db.createNotification(
      ...
      `Head of Department (${hodName}) has approved and endorsed your clearance form. Forwarded to Admin for final approval and certificate issuance.`,
      'success'
    );
  }

  // Notify Principal / Admin users
  const adminUsers = db.users.filter(u => u.role === 'ADMIN');
  for (const admin of adminUsers) {
    db.createNotification(
      admin.id,
      'Clearance Application Ready for Principal Approval',
      `Student ${student ? student.full_name : 'Applicant'} (${student ? student.register_number : ''}) clearance has been fully verified by allocated staff and endorsed by HOD (${hodName}). Ready for Admin review.`,
      'info'
    );
  }
  ...
  res.json({
    message: 'Official HOD Endorsement signed successfully. Forwarded to Admin.',
    request: noDueReq
  });
});
...
apiRouter.post(['/hod/requests/:id/clear-subject', '/staff/requests/:id/clear-subject'], authMiddleware, requireRole(['HOD', 'STAFF', 'ADMIN']), (req: AuthRequest, res: Response) => {
  ...
  if (!slot) {
    return res.status(400).json({ detail: 'Slot name is required (e.g., Sub 1, Lab 2, COM-LIB).' });
  }

  const student = db.students.find(s => s.id === noDueReq.student_id);
  if (!noDueReq.subjects || !Array.isArray(noDueReq.subjects) || noDueReq.subjects.length === 0) {
    noDueReq.subjects = defaultSubjectsForStudent(student, noDueReq.semester, true);
  }
  if (!noDueReq.labs || !Array.isArray(noDueReq.labs) || noDueReq.labs.length === 0) {
    noDueReq.labs = defaultLabsForStudent(student, noDueReq.semester, true);
  }
  if (!noDueReq.common_nodes || !Array.isArray(noDueReq.common_nodes) || noDueReq.common_nodes.length === 0) {
    noDueReq.common_nodes = defaultCommonNodesForStudent(student, true);
  }
  ...
  // Strict staff authorization check: Staff can only clear items allocated to them
  if (req.user!.role === 'STAFF') {
    const allReqItems = [
      ...(noDueReq.subjects || []),
      ...(noDueReq.labs || []),
      ...(noDueReq.common_nodes || [])
    ];
    const targetItem = allReqItems.find((item: any) => {
      const subCode = (item.code || '').toLowerCase();
      const slotName = (item.slot || '').toLowerCase();
      const comKey = (item.category_key || '').toLowerCase();
      const sLower = slot.toLowerCase();
      return slotName === sLower || subCode === sLower || comKey === sLower || item.name?.toLowerCase().includes(sLower);
    });

    if (!targetItem) {
      return res.status(404).json({
        detail: `Clearance node '${slot}' not found in this request.`
      });
    }

    if (!isItemAllocatedToStaff(targetItem, staff, req.user!.role)) {
      return res.status(403).json({
        detail: `Unauthorized: You are not the allocated staff member for '${targetItem.name || slot}'. Only the allocated staff can clear this node.`
      });
    }
  }
  ...
  if (noDueReq.common_nodes && Array.isArray(noDueReq.common_nodes)) {
    for (const com of noDueReq.common_nodes) {
      const comCode = (com as any).code || '';
      const comKey = (com as any).category_key || '';
      if (
        com.slot?.toLowerCase() === slot.toLowerCase() ||
        comCode.toLowerCase() === slot.toLowerCase() ||
        comKey.toLowerCase() === slot.toLowerCase() ||
        com.name?.toLowerCase().includes(slot.toLowerCase())
      ) {
        com.dues_status = 'No Dues';
        com.faculty_name = staffName;
        com.signature_date = todayStr;
        updated = true;

        if (noDueReq.signatories) {
          if (comKey === 'library' || comCode.includes('LIB') || com.name?.toLowerCase().includes('library')) {
            noDueReq.signatories.library = { signed: true, name: staffName, date: todayStr, status: 'No Due' };
          } else if (comKey === 'accounts' || comCode.includes('ACC') || com.name?.toLowerCase().includes('account')) {
            noDueReq.signatories.office_accounts = { signed: true, name: staffName, date: todayStr, status: 'No Dues' };
          } else if (comKey === 'transport' || comCode.includes('TRN') || com.name?.toLowerCase().includes('transport')) {
  ...
  if (student) {
    db.createNotification(
      ...
      `Your clearance for ${slot} has been accepted with 'No Dues' by ${staffName}.`,
      'success'
    );
  }

  // Check if all clearance nodes are now complete
  const allSubCleared = (noDueReq.subjects || []).every((s: any) => isNodeClearedStatus(s.dues_status));
  const allLabCleared = (noDueReq.labs || []).every((l: any) => l.name === '-' || isNodeClearedStatus(l.dues_status));
  const allComCleared = (noDueReq.common_nodes || []).every((c: any) => isNodeClearedStatus(c.dues_status));
  const allClearedNow = allSubCleared && allLabCleared && allComCleared;

  if (allClearedNow && student) {
    // Notify HOD
    const dept = db.departments.find(d => d.id === student.department_id);
    const hodUsers = db.users.filter(u => u.role === 'HOD');
    for (const h of hodUsers) {
      db.createNotification(
        h.id,
        'Clearance Ready for HOD Endorsement',
        `All allocated subjects, labs, and nodes for student ${student.full_name} (${student.register_number}) have been verified and cleared by staff. Ready for HOD endorsement.`,
        'info'
      );
    }
  }
  ...
  res.json({
    message: `Clearance accepted with No Dues for ${slot} by ${staffName}.${allClearedNow ? ' All subjects and labs are now cleared! Ready for HOD endorsement.' : ''}`,
    request: noDueReq,
    all_cleared: allClearedNow
  });
});

apiRouter.post([
  '/hod/requests/:id/mark-subject-due',
  '/staff/requests/:id/mark-subject-due',
  '/hod/requests/:id/reset-subject',
  '/staff/requests/:id/reset-subject'
], authMiddleware, requireRole(['HOD', 'STAFF', 'ADMIN']), (req: AuthRequest, res: Response) => {
  const reqId = Number(req.params.id);
  const noDueReq = db.noDueRequests.find(r => r.id === reqId);
  if (!noDueReq) {
    return res.status(404).json({ detail: 'No Due request not found' });
  ...
  const { slot, amount, description, action, status } = req.body;
  if (!slot) {
    return res.status(400).json({ detail: 'Slot name is required.' });
  }

  const isReset = action === 'reset' || status === 'Pending Review' || req.path.includes('reset-subject');
  const dueText = isReset
    ? 'Pending Review'
    : (amount && Number(amount) > 0 ? `Due: ₹${amount}` : (description || 'Pending Due'));
  const sigText = isReset ? '-' : (amount && Number(amount) > 0 ? `Due: ₹${amount}` : 'Action Required');
  const dateText = isReset ? '-' : new Date().toLocaleDateString('en-GB');
  ...
  if (noDueReq.subjects && Array.isArray(noDueReq.subjects)) {
    for (const sub of noDueReq.subjects) {
      ...
      if (sub.slot?.toLowerCase() === slot.toLowerCase() || subCode.toLowerCase() === slot.toLowerCase()) {
        ...
        (sub as any).staff_signature = sigText;
        (sub as any).date = dateText;
      }
    }
  }
  ...
  if (noDueReq.labs && Array.isArray(noDueReq.labs)) {
    for (const lab of noDueReq.labs) {
      ...
      if (lab.slot?.toLowerCase() === slot.toLowerCase() || labCode.toLowerCase() === slot.toLowerCase()) {
        ...
        (lab as any).staff_signature = sigText;
        (lab as any).date = dateText;
      }
    }
  }
  ...
  if (noDueReq.common_nodes && Array.isArray(noDueReq.common_nodes)) {
    for (const com of noDueReq.common_nodes) {
      ...
      if (
        ...
        com.dues_status = dueText;
        (com as any).staff_signature = sigText;
        (com as any).date = dateText;

        if (noDueReq.signatories) {
          if (comKey === 'library' || comCode.includes('LIB') || com.name?.toLowerCase().includes('library')) {
            noDueReq.signatories.library = { signed: false, name: '-', date: '-', status: dueText };
          } else if (comKey === 'accounts' || comCode.includes('ACC') || com.name?.toLowerCase().includes('account')) {
            noDueReq.signatories.office_accounts = { signed: false, name: '-', date: '-', status: dueText };
          } else if (comKey === 'transport' || comCode.includes('TRN') || com.name?.toLowerCase().includes('transport')) {
  ...
  // Reset HOD endorsement if it was endorsed, since a subject now has a due or is pending
  if (noDueReq.signatories?.hod) {
    noDueReq.signatories.hod.signed = false;
    noDueReq.signatories.hod.status = 'pending';
    noDueReq.signatories.hod.remarks = isReset ? 'Clearance reset; pending staff re-verification' : `Pending due: ${dueText}`;
  }
  noDueReq.hod_approved_at = undefined;
  noDueReq.hod_approved_by = undefined;
  if (noDueReq.status === 'approved') {
    noDueReq.status = 'under_review';
  }

  // If a monetary due was specified, also create a record in student dueRecords if not existing
  if (!isReset && amount && Number(amount) > 0) {
    const student = db.students.find(s => s.id === noDueReq.student_id);
    if (student) {
      const cat = db.dueCategories.find(c => c.name.toLowerCase().includes('breakage') || c.name.toLowerCase().includes('lab') || c.name.toLowerCase().includes('tuition')) || db.dueCategories[0];
      const dept = db.departments.find(d => d.id === student.department_id) || db.departments[0];
      const newDue: DueRecord = {
        id: db.dueRecords.length > 0 ? Math.max(...db.dueRecords.map(d => d.id)) + 1 : 1,
        student_id: student.id,
        department_id: dept.id,
        category_id: cat ? cat.id : 1,
        amount: Number(amount),
        status: 'pending',
        description: description || `Outstanding fine/due for ${slot}`,
        remarks: `Recorded via clearance review on ${new Date().toLocaleDateString('en-GB')}`,
        created_by: req.user!.id,
        updated_by: req.user!.id,
        created_at: new Date().toISOString()
      };
  ...
  db.saveToFile();
  res.json({
    message: isReset ? `Clearance status reset to Pending Review for ${slot}.` : `Due marked for ${slot} (${dueText}).`,
    request: noDueReq
  });
});
